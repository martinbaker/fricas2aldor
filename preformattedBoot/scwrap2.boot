...
DEFPARAMETER($compiler_InteractiveFrame,addBinding('$DomainsInScope,[["FLUID", :true],["special", :(COPY_-TREE $InitialDomainsInScope)]],addBinding('$Information, nil,makeInitialModemapFrame())))
...
spad_syntax_error(wanted, parsing) ==
    ...
    if $prev_line then
        FORMAT(true, '"~&The prior line was:~%~%~5D> ~A~%~%",$prev_line_number, $prev_line)
    if $curent_line then
        FORMAT(true, '"~&The current line is:~%~%~5D> ~A~%~%",$curent_line_number, $curent_line)
    TOKEN_-STACK_-SHOW()
...
$expression_nostarters := [ "ARROW", "BACKSET", "BECOMES", "COLON", _
    "COMMA", "DEF", "ELSE", "EXIT", "GIVES", "MDEF", "SEMICOLON", _
...
ntokreader(token) ==
    ...
    if $toklst then
        ...
        if type1 = "key" and sym = "SETTAB" and $toklst then
            ...
            ntype1 := first(ntok1)
            nsym := ntok1.1
            if ntype1 = "key" and nsym in ["then", "else","COMMA", "SEMICOLON"] then
...
fakeloopInclude0(st, name, n) ==
    ...
    fakeloopProcess(n,next(function insertpile,next(function lineoftoks,$lines)))
    nil
