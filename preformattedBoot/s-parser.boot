...
repetition(delimiter, fn) ==
    ...
    repeat
        if delimiter then
            if not(match_symbol(delimiter)) then return nil
            -- break loop
            MUST(FUNCALL fn)
        else
            if not(FUNCALL fn) then return nil
            -- break loop
        val := [pop_stack_1(), :val]
...
parse_Infix() ==
    ...
    parse_TokTail()
    MUST parse_Expression()
    push_reduction("parse_Infix",[pop_stack_2(), pop_stack_2(), pop_stack_1()])
...
parse_TokTail() ==
    current_symbol() ~= "$" => nil
    not(OR(match_next_token("IDENTIFIER", NIL), next_symbol() = "%",next_symbol() = "(")) => nil
    -- )
...
parse_Qualification() ==
    ...
    MUST parse_Primary1()
    push_reduction("parse_Qualification",dollarTran(pop_stack_1(), pop_stack_1()))
...
parse_Loop() ==
    OR(AND(repetition(nil, FUNCTION parse_Iterator),expect_symbol "repeat", MUST parse_Expr 110,push_lform1("REPEAT", [:pop_stack_2(), pop_stack_1()])),_AND(expect_symbol "repeat", MUST parse_Expr 110,push_form1("REPEAT", pop_stack_1())))
...
parse_NudPart($RBP) ==
    AND(OR(parse_Operation("Nud", $RBP), parse_Reduction(), parse_Form()),push_reduction("parse_NudPart", pop_stack_1()))

parse_Operation($ParseMode, $RBP) ==
    match_current_token("IDENTIFIER", NIL) => nil
    tmptok := current_symbol()
    SYMBOLP(tmptok) and GET(tmptok, $ParseMode) and $RBP < parse_leftBindingPowerOf(tmptok, $ParseMode) =>
        $RBP := parse_rightBindingPowerOf(tmptok, $ParseMode)
        parse_getSemanticForm($ParseMode,ELEMN(GET(tmptok, $ParseMode), 5, NIL))
...
parse_ReductionOp() ==
    ...
    AND(SYMBOLP(cur_sym), GET(cur_sym, "Led"),match_next_token("KEYWORD", "/"),push_reduction("parse_ReductionOp", cur_sym),ACTION advance_token(), ACTION advance_token())
...
parse_Selector() ==
    not(match_symbol ".") => nil
    MUST parse_Primary()
    push_reduction("parse_Selector",[pop_stack_2(), pop_stack_1()])
...
--mjb
parse_Primary1() ==
  OR(AND(parse_VarForm(),OPTIONAL AND(current_token_is_nonblank(),current_symbol() = "(", MUST parse_Enclosure(),push_reduction("parse_Primary1",[pop_stack_2(), pop_stack_1()]))),parse_String(), parse_IntegerTok(),parse_FormalParameter(),AND(symbol_is? "'",MUST AND(match_symbol "'", MUST parse_Expr 999,push_form1("QUOTE", pop_stack_1()))),parse_Sequence(), parse_Enclosure())

parse_Float() == parse_SPADFLOAT()

parse_Enclosure1(closer) ==
    MUST OR(AND(parse_Expr 6, expect_symbol(closer)),AND(expect_symbol(closer), push_form0("@Tuple")))
...
parse_Data() == AND(ACTION(advance_token()),OR(parse_IDENTIFIER(), parse_KEYWORD()),push_form1("QUOTE", pop_stack_1()))
...
parse_Sequence1() ==
    ...
    OPTIONAL AND(parse_IteratorTail(),push_lform1("COLLECT", [:pop_stack_1(),pop_stack_1()]))
